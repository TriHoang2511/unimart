...
@section('content')
    <div id="content" class="container-fluid py-4">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                <h5 class="m-0 font-weight-bold text-primary"><i class="fas fa-user-tag mr-2"></i>Cập nhật vai trò</h5>
                ...
                <form method="POST" action="{{ route('admin.role.update', $role->id) }}">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" name="description" id="description"
                                    rows="3" placeholder="Mô tả ngắn gọn chức năng của vai trò này">{{ old('description', $role->description) }}</textarea>
                                @error('description')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                                                ...
                                                <div class="col-md-3">
                                                    <div class="form-check mb-2">
                                                        <input class="form-check-input permission-checkbox" type="checkbox"
                                                            ...
                                                            data-module="{{ $moduleName }}" id="p-{{ $permission->id }}"
                    ...
                    <div class="mt-4 border-top pt-3 text-right">
                        <button type="submit" class="btn btn-primary px-5 shadow-sm font-weight-bold">
                            <i class="fas fa-edit mr-1"></i> Cập nhật vai trò
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        // Tự động kiểm tra trạng thái các nút "Check All" khi trang load xong
        document.querySelectorAll('.check-all-module').forEach(moduleCheckbox => {
